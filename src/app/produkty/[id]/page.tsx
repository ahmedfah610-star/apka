...
export default function StronaProduktu({ params }: { params: { id: string } }) {
  ...
  const podobne = PRODUKTY.filter((x) => x.kategoria === p.kategoria && x.id !== p.id).slice(0, 4);
    ...
    <div className="overflow-x-hidden">
      ...
      <div className="mx-auto grid max-w-content grid-cols-1 gap-12 px-6 pb-20 md:grid-cols-2 md:px-12">
        ...
        <div className="flex flex-col">
          ...
          {p.rozmiary && p.rozmiary.length > 0 ? (
            <div className="mb-7">
              <h3 className="mb-3 text-[13px] font-semibold tracking-wide text-ink-2">DOSTĘPNE ROZMIARY</h3>
              <div className="flex flex-wrap gap-2">
                {p.rozmiary.map((s) => (
                  <span
                    key={s}
                    className="cursor-pointer border border-linia-2 px-3.5 py-2 text-[13px] font-medium transition-colors hover:border-ink"
                  >
                    {s}
                  </span>
                ))}
          ...
          <button className="mb-8 w-full bg-ink px-8 py-4 text-[13px] font-semibold tracking-wide text-tlo transition-colors hover:bg-akcent sm:w-auto">
            DODAJ DO KOSZYKA
          </button>
