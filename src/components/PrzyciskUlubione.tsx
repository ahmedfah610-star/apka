...
export function PrzyciskUlubione({ id, wariant = "karta" }: { id: string; wariant?: "karta" | "produkt" }) {
  ...
  if (wariant === "produkt") {
    return (
      <button
        ...
        className={`flex items-center justify-center gap-2 border px-5 py-4 text-[13px] font-semibold tracking-wide transition-colors ${
          aktywne ? "border-akcent bg-akcent/5 text-akcent" : "border-linia-2 text-ink hover:border-ink"
        }`}
      >
        <Serce pelne={aktywne} />
  ...
  return (
    <button
      ...
      aria-pressed={aktywne}
      className="absolute right-2.5 top-2.5 z-10 flex h-9 w-9 items-center justify-center rounded-full bg-white/90 text-ink shadow-sm backdrop-blur transition-colors hover:bg-white"
    >
      <Serce pelne={aktywne} />
    </button>
  );
